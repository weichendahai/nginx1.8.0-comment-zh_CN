...
struct ngx_buf_s {
	...
	// 处理文件数据
    off_t            file_pos;   //告知需要处理的文件数据的起始位置j
    off_t            file_last;  //告知需要处理的文件数据的结束位置

	// 处理内存数据
    u_char          *start;         /* start of buffer */ //当一整块内存被包含在多个buf中的时候，那么这些buf里面的start和end都指向这块内存的起始位置和终止位置，和pos不同，pos会大于等于start
    u_char          *end;           /* end of buffer */  //见start分析，和last不同，last会小于等于end
...
struct ngx_output_chain_ctx_s {
    ...
    ngx_chain_t                 *busy;    //保存了还未发送的chainjj
